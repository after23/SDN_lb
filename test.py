...
class LoadBalancer(app_manager.RyuApp):
    ...
    H5_mac = "00:00:00:11:11:11"          # Host 5's mac
    H5_ip = "10.0.0.11"                    # Host 5's IP   
    H6_mac = "00:00:00:22:22:22"          # Host 6's mac
    H6_ip = "10.0.0.22"                    # Host 6's IP
    H7_ip = "10.0.0.33"                    
    H7_mac = "00:00:00:33:33:33"
    next_server = ""      # Stores the IP of the  next server to use in round robin manner
    current_server = ""   # Stores the current server's IP
    ip_to_port = {H5_ip: 1, H6_ip: 2, H7_ip: 3}
    ...
    def arp_response(self, datapath, packet, etherFrame, ofp_parser, ofp, in_port):
        ...
        if dstIp != self.H5_ip and dstIp != self.H6_ip and dstIp != self.H7_ip:
            if self.next_server == self.H5_ip:
                srcMac = self.H5_mac
                self.next_server = self.H6_ip
            elif self.next_server == self.H6_ip:
                srcMac = self.H6_mac
                self.next_server = self.H7_ip
            else :
                srcMac = self.H7_mac
                self.next_server = self.H5_ip
        else:
